...
class Graph:
    ...
    def prepareNet(self, size):
        ...
        for i in range(size):
            ...
            for j in range(size):
                row.append(chr(b'a'[0]+ i))
                i += 1
            nodeList.append(row)
        
        for row in nodeList:
            for node in row:
                self.add_vertex(node, NodePoint())
                
        #poziome
        for i in range(size):
            for j in range(size - 1):
                self.add_edge(nodeList[i][j], nodeList[i][j+1], 1)
        
        for i in range(size):
            for j in range(size - 1):
                self.add_edge(nodeList[j][i], nodeList[j+1][i], 1)
    ...
    def getShortest(self, nodeA, nodeB):
        self.dijkstra( graph.get_vertex(nodeA), graph.get_vertex(nodeB))

        target = graph.get_vertex(nodeB)
        path = [target.get_id()]
