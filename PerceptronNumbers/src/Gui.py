...
class MainWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.perceptrons = [Perceptron(myDigit = i, examples = self.examples, weightsCount = ENTRANCES) for i in range(PERCEPTRONS)]
        for perc in self.perceptrons:
            perc.setWeights(self.xmlParser.getWeights(perc.getMyDigit()),self.xmlParser.getTheta(perc.getMyDigit()))
    ...
    def setupUI(self):                
        ...
        self.learnButton.clicked.connect(self.learnPerceptrons)
    ...
    def testPixels(self):
        result = [perc.isThatYourNumber(self.pixelMap) for perc in self.perceptrons]
        print(result)
    ...
    def learnPerceptrons(self):
        self.learnLabel.setText("<b><FONT SIZE = 4>Status uczenia: w toku...</b>")
        self.learnLabel.show()
        percPool = PerceptronPool(self.perceptrons, self.examples)
        beforeTime = time.process_time()
        self.perceptrons = percPool.learnPerc()
        learnTime = elapsed_time = time.process_time() - beforeTime
        for perc in self.perceptrons:
            self.xmlParser.setWeights(perc.getMyDigit(), perc.getTheta(), perc.getWeights()) 
        self.xmlParser.saveXml()
        #self.perceptrons[0].start()
        #self.perceptrons[0].join()
